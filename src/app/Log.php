...
class Log
{
     ...
     * @var integer
     */
    private $logStatus         = self::STATUS_LOG_CLOSED;
     ...
     * @var integer
     */
    private $severityThreshold = self::INFO;
    ...
    public function __construct($logDirectory, $severity)
    {
        ...
        if (file_exists($this->logFilePath) && !is_writable($this->logFilePath)) {
            ...
            $this->messageQueue[] = $this->messages['writefail'];
            return;
        }

        if (($this->fileHandle = fopen($this->logFilePath, 'a'))) {
            $this->logStatus = self::STATUS_LOG_OPEN;
            $this->messageQueue[] = $this->messages['opensuccess'];
        } else {
            $this->logStatus = self::STATUS_OPEN_FAILED;
            $this->messageQueue[] = $this->messages['openfail'];
        }
    }
    ...
    {
        return array_pop($this->messageQueue);
    }
    ...
    public function appendLog($line, $severity, $args = self::NO_ARGUMENTS)
    {
        if ($this->severityThreshold >= $severity) {
            $status = $this->getTimeLine($severity);
    ...
    {
        if ($this->logStatus == self::STATUS_LOG_OPEN
